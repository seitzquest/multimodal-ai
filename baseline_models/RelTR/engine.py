...
from lib.evaluation.Image_Translation import image_translanting, get_co_occurence_matrix
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    co_occurrence_matrix = get_co_occurence_matrix(data_loader)

    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        samples = samples.to(device)
        samples = image_translanting(samples, mode="untrained_object", matrix=co_occurrence_matrix, targets=targets)
...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, mode, scaling, patch_strategy, obj_in_rl, args):
    model.eval()
    criterion.eval()
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, 100, header)):
        ...
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        samples = image_translanting(samples, mode=mode, matrix=co_occurrence_matrix, targets=targets, scaling=scaling, patch_strategy=patch_strategy, obj_in_rl=obj_in_rl)
